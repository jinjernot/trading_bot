from binance.enums import *
from src.trade import place_order
from data.indicators import *
from config.settings import *

async def check_fib_pullback_long_entry(symbol, df_15m, df_4h, usdt_balance):
    """
    Checks for a LONG buying opportunity based on a pullback in an uptrend.
    """
    adx_4h = df_4h['ADX'].iloc[-1]
    price_sma_4h = df_4h['price_sma_50'].iloc[-1]
    last_close_4h = df_4h['close'].iloc[-1]

    is_strong_uptrend = adx_4h > 25 and last_close_4h > price_sma_4h
    if not is_strong_uptrend:
        return

    swing_low, swing_high, fib_levels = find_swing_points_and_fib(df_15m)
    if not fib_levels:
        return

    last_close_15m = df_15m['close'].iloc[-1]
    target_fib_level = fib_levels['0.618']
    
    if last_close_15m <= target_fib_level and last_close_15m > swing_low:
        print(f"🚀🚀🚀 LONG FIBONACCI SIGNAL FOR {symbol} 🚀🚀🚀")
        print(f"Price pulled back to 0.618 level (${target_fib_level:.4f}).")
        
        atr_value = df_15m['atr'].iloc[-1]
        await place_order(
            symbol=symbol, side=SIDE_BUY, usdt_balance=usdt_balance,
            reason_to_open=f"Fibonacci pullback to 0.618 level",
            stop_loss_price=swing_low * 0.995,
            atr_value=atr_value, df=df_15m,
            support_4h=swing_low, resistance_4h=swing_high, adx_value=adx_4h
        )
        return True
    return False

async def check_fib_retrace_short_entry(symbol, df_15m, df_4h, usdt_balance):
    """
    Checks for a SHORT selling opportunity based on a retrace in a downtrend.
    """
    adx_4h = df_4h['ADX'].iloc[-1]
    price_sma_4h = df_4h['price_sma_50'].iloc[-1]
    last_close_4h = df_4h['close'].iloc[-1]

    is_strong_downtrend = adx_4h > 25 and last_close_4h < price_sma_4h
    if not is_strong_downtrend:
        return

    swing_low, swing_high, fib_levels = find_swing_points_and_fib_short(df_15m)
    if not fib_levels:
        return

    last_close_15m = df_15m['close'].iloc[-1]
    target_fib_level = fib_levels['0.618']
    
    if last_close_15m >= target_fib_level and last_close_15m < swing_high:
        print(f"🔥🔥🔥 SHORT FIBONACCI SIGNAL FOR {symbol} 🔥🔥🔥")
        print(f"Price retraced to 0.618 level (${target_fib_level:.4f}).")

        atr_value = df_15m['atr'].iloc[-1]
        await place_order(
            symbol=symbol, side=SIDE_SELL, usdt_balance=usdt_balance,
            reason_to_open=f"Fibonacci retrace to 0.618 level",
            stop_loss_price=swing_high * 1.005,
            atr_value=atr_value, df=df_15m,
            support_4h=swing_low, resistance_4h=swing_high, adx_value=adx_4h
        )
        return True
    return False